...
export default function SalvadosAdminPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Usamos FormData para enviar os campos de texto e também os ficheiros de imagem
    ...
    <main className="min-h-screen bg-zinc-100 px-6 py-10">
      <div className="mx-auto max-w-6xl">
        ...
        <form onSubmit={handleSubmit} className="mb-12 rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm">
          <h2 className="mb-6 text-2xl font-black">Novo salvado</h2>
          <div className="grid gap-6 md:grid-cols-2">
            <div>
              <label className="mb-2 block font-semibold">Referência *</label>
              <input name="reference" value={form.reference} onChange={handleChange} placeholder="Ex.: SALV-001" required className="w-full rounded-xl border border-zinc-300 px-4 py-3 outline-none focus:border-red-500" />
            </div>
            <div>
              <label className="mb-2 block font-semibold">Título / Descrição Curta *</label>
              <input name="title" value={form.title} onChange={handleChange} placeholder="Ex.: BMW 320d Para Peças" required className="w-full rounded-xl border border-zinc-300 px-4 py-3 outline-none focus:border-red-500" />
            </div>
            <div>
              <label className="mb-2 block font-semibold">Marca *</label>
              <select name="brandId" value={form.brandId} onChange={handleChange} required className="w-full rounded-xl border border-zinc-300 bg-white px-4 py-3">
                <option value="">Selecionar marca</option>
                {brands.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            </div>
            <div>
              <label className="mb-2 block font-semibold">Modelo *</label>
              <select name="modelId" value={form.modelId} onChange={handleChange} required disabled={!form.brandId} className="w-full rounded-xl border border-zinc-300 bg-white px-4 py-3 disabled:bg-zinc-100">
                <option value="">{form.brandId ? "Selecionar modelo" : "Escolher primeiro a marca"}</option>
                {modelosDisponiveis.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
              </select>
            </div>
            <div>
              <label className="mb-2 block font-semibold">Ano</label>
              <input name="year" type="number" value={form.year} onChange={handleChange} placeholder="Ex.: 2020" className="w-full rounded-xl border border-zinc-300 px-4 py-3" />
            </div>
            <div>
              <label className="mb-2 block font-semibold">Quilómetros</label>
              <input name="kilometers" type="number" value={form.kilometers} onChange={handleChange} placeholder="Ex.: 145000" className="w-full rounded-xl border border-zinc-300 px-4 py-3" />
            </div>
            <div>
              <label className="mb-2 block font-semibold">Preço (€)</label>
              <input name="price" type="number" step="0.01" value={form.price} onChange={handleChange} placeholder="Ex.: 4500.00" className="w-full rounded-xl border border-zinc-300 px-4 py-3" />
            </div>
            <div className="md:col-span-2">
              <label className="mb-2 block font-semibold">Fotografias do veículo</label>
              <input type="file" multiple accept="image/*" onChange={(e) => setImagens(e.target.files)} className="w-full rounded-xl border border-zinc-300 px-4 py-3 file:mr-4 file:rounded-lg file:border-0 file:bg-red-50 file:px-4 file:py-2 file:font-semibold file:text-red-700 hover:file:bg-red-100" />
            </div>
          </div>
        ...
        <section>
          <h2 className="mb-5 text-2xl font-black">Salvados existentes</h2>
